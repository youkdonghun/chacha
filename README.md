...
새 EXE를 직접 받은 경우에는 먼저 기존 앱을 완전히 종료한 뒤 **v1.3.0** 파일을 실행하세요.
...
v1.1 이하에는 이 메뉴가 없으므로 v1.3 EXE를 한 번 직접 다운로드한 뒤 사용할 수 있습니다.
...
입력란에 포커스가 있는 동안에는 다른 앱이 등록한 키도 조합으로 읽습니다. 해당 키를 저장한 뒤의 전역 등록 충돌은 별도로 확인합니다.
...
v1.3.0은 단축키 녹화와 입력 안내, 명시적인 캡처 방식 선택·진단, 둥근 버튼·설정 화면을 개선한 버전입니다. **Snipaste 전체 제품과 완전한 1:1 일치를 보증하는 버전은 아닙니다.**
...
결과: `dist\ChachaCapture.exe`, `dist\ChachaCapture-1.3.0-win-x64.zip`, SHA-256, 자체 검사 결과.
[검증 기록](docs/VALIDATION.md)과 [변경점](docs/RELEASE-v1.3.0.md)을 참고하세요.
