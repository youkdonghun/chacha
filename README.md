# Chacha Capture

Windows x64용 캡처·주석·화면 고정 프로그램입니다. Snipaste Windows 무료판의 조작 흐름을 기준으로 직접 구현했습니다. 원본 소프트웨어의 코드·로고·리소스는 사용하지 않습니다.

## 다운로드 및 실행

[최신 릴리스](https://github.com/youkdonghun/chacha/releases/latest)에서 **ChachaCapture.exe**를 받아 실행하세요.
저장소와 배포 파일은 공개되어 있어 GitHub 로그인 없이 다운로드할 수 있습니다.
Windows 10/11 x64와 .NET Framework 4.8을 사용합니다. 별도 설치나 관리자 권한 없이 EXE 하나로 실행됩니다.
창을 닫으면 트레이에서 계속 실행합니다. 완전히 종료하려면 트레이 우클릭 → 종료를 사용하세요.
새 EXE를 직접 받은 경우에는 먼저 기존 앱을 완전히 종료한 뒤 **v1.4.1** 파일을 실행하세요.

## 앱 안에서 업데이트

대시보드의 **업데이트** 또는 트레이 메뉴의 **업데이트 확인**을 누릅니다.
GitHub 최신 정식 버전이 있으면 **다운로드 → 설치하고 다시 시작**으로 현재 EXE를 교체합니다.
다운로드한 파일의 SHA-256과 64비트 실행 파일 형식을 확인하며, 교체에 실패하면 기존 파일을 복구합니다.
설정과 캡처·플로팅 이미지는 유지합니다. 확인·다운로드는 취소할 수 있으며 새 버전을 자동으로 설치하지 않습니다.
v1.1 이하에는 이 메뉴가 없으므로 v1.4 EXE를 한 번 직접 다운로드한 뒤 사용할 수 있습니다.
공개 저장소이므로 업데이트 확인과 다운로드에 GitHub 로그인이나 별도 토큰이 필요하지 않습니다.
이미 GitHub CLI에 로그인했거나 `GH_TOKEN` / `GITHUB_TOKEN`이 설정되어 있으면 해당 인증을 사용합니다.
인증값은 실행 파일·설정·로그에 저장하지 않습니다.

## 캡처

| 동작 | 조작 |
|---|---|
| 캡처 시작 | **F1**, 트레이 왼쪽 클릭 |
| 영역 선택 | 드래그; 자동 감지를 켠 경우 창·요소 클릭도 가능 |
| 복사하고 닫기 | **Ctrl+C**, 복사 버튼 (자동 플로팅 설정과 관계없이 닫기) |
| 복사하고 자동 플로팅 설정에 따라 완료 | **Enter**, 영역 더블클릭 |
| 바로 플로팅 | 설정한 플로팅 단축키(기본 **F3**), **Ctrl+T**, 영역 가운데 클릭, **화면에 띄우기** 버튼 |
| 저장 / 빠른 저장 / 인쇄 | **Ctrl+S / Ctrl+Shift+S / Ctrl+P** |
| 전체 화면 / 마지막 성공 영역 | **Ctrl+A / R** |
| 캡처 기록 이전 / 다음 | **, / .** |
| 창 / UI 요소 감지 전환 | 자동 감지가 켜져 있을 때 **Tab**, 선택 전 휠로 겹친 영역 탐색 |
| 영역 이동 / 확대 / 축소 | **방향키 / Ctrl+방향키 / Shift+방향키** |
| 커서 1픽셀 이동 | **W A S D** |
| 캡처한 커서 표시 전환 | **`** |
| 확대경 / 색상 형식 / 색상 복사 | **Alt / Shift / C** |
| 주석 도구 모음 표시 전환 | **Space** |
| 크기 조절 중 영역 이동 | 왼쪽 버튼을 누른 채 **Space** |
| 취소 / 영역 다시 선택 | **Esc / 오른쪽 클릭** |

트레이 메뉴에는 전체 화면·3초 지연·최근 영역·좌표 및 크기 지정 캡처와 화이트보드도 있습니다.
선택 영역 아래 도구 모음의 주석 도구를 누르면 같은 화면 위에서 편집합니다.
설정 → 캡처에서 **창과 화면 요소의 영역 자동 감지**를 끄면 직접 드래그한 영역만 선택합니다.
이 상태에서는 창·요소 강조와 클릭 선택을 하지 않으며 Tab이나 휠로 감지가 다시 켜지지 않습니다.
**Ctrl+C**나 복사 버튼으로 완료하면 캡처·주석 화면이 닫히며 새 플로팅 창을 만들지 않습니다.
이미 떠 있는 이미지나 그 이미지의 편집창에서도 Ctrl+C로 복사하면 해당 플로팅 창까지 닫힙니다. 다른 플로팅 창은 그대로 유지합니다.
**Enter**, 영역 더블클릭, 저장으로 완료하면 자동 플로팅 설정을 따릅니다. 설정 → 캡처에서 자동 플로팅을 끌 수 있으며, Ctrl+T는 이 설정과 관계없이 플로팅합니다.
캡처·편집 중 **플로팅** 명령은 현재 이미지만 화면에 띄웁니다. 클립보드 복사, 빠른 저장, 캡처 기록 추가를 함께 실행하지 않습니다.
재실행 복원에 필요한 플로팅 창 상태와 이미지 캐시는 별도로 보관합니다. Ctrl+C로 닫은 플로팅 이미지도 닫은 이미지 복구 기능으로 다시 열 수 있습니다.
클립보드 복사가 실패하면 이미지를 버리지 않고 기존 창을 유지하거나 캡처 이미지를 플로팅 창으로 보관합니다.
설정 → 캡처 → **캡처 방식**에서 다음 중 하나를 선택합니다.

| 방식 | 동작 |
|---|---|
| 자동 · 호환 방식 우선 (기본) | 표준 GDI로 화면을 읽고, API 오류가 생기면 DXGI를 시도합니다. |
| 호환 방식 · GDI | GDI만 사용합니다. 다른 캡처 앱과 함께 사용할 때 선택할 수 있습니다. |
| GPU 방식 · DXGI | Desktop Duplication만 사용합니다. 선택한 GPU 화면을 다른 방식의 픽셀로 바꾸지 않습니다. |

화면이 실제와 다르면 캡처를 취소하고 방식을 바꿔 다시 시도하세요.
같은 설정 탭의 **최근 캡처 진단 복사**로 앱 버전, 사용한 방식, 화면 크기와 API 오류를 복사할 수 있습니다.
진단에는 이미지 내용이 들어가지 않습니다. 완전한 단색 화면은 미리보기 확인 안내를 표시하며, 색상만으로 캡처를 실패 처리하거나 다른 이미지로 교체하지 않습니다.
GPU가 커서를 화면에 포함한 경우에는 커서를 중복해서 그리지 않습니다. 커서를 빼야 한다면 호환 방식을 사용하세요.
새 캡처 전에 열려 있던 편집창은 캡처에서 제외합니다. 캡처 취소 시 복원하고, 완료 시 작업 표시줄로 최소화해 편집 내용을 보존합니다.

## 주석

- 사각형, 타원, 선, 화살표, 꺾은선, 펜, 형광펜, 문자, 순서 번호
- 모자이크, 흐림, 지우개, 자르기
- 색상·선 굵기·채움·알파, 문자 서체·굵게·기울임·회전
- 주석 선택 후 이동·크기 변경, 문자 더블클릭 수정
- **Ctrl+Z** 실행 취소, **Ctrl+Y** 다시 실행, **Ctrl+Shift+Z** 모든 편집 초기화
- **Shift**로 정사각형·원·각도 제한, **Tab**으로 선·화살표 전환
- PNG/JPEG/BMP 저장, PNG 클립보드 형식으로 투명도 보존

캡처 위에 표시되는 그리기 도구도 **선택·사각·타원·화살표·펜·문자**처럼 한국어 이름을 표시합니다.
문자 입력창은 설명·입력 영역·버튼 사이에 여백을 두고, 여러 줄 입력과 스크롤을 지원합니다.
화이트보드에서는 도구 모음이 처음에 숨겨져 있습니다. **Space**로 표시하며, 종료는 도구 모음의 닫기 버튼을 사용합니다.

## 화면 고정

| 동작 | 조작 |
|---|---|
| 현재 선택·편집 이미지 플로팅 / 최근 이미지 복구·클립보드 붙여넣기 | 설정한 플로팅 단축키(기본 **F3**), 트레이 가운데 클릭 |
| 현재 그룹 전체 숨기기 / 표시 | **Shift+F3** |
| 커서 아래 이미지 클릭 통과 | **Ctrl+Alt+F3** |
| 다음 이미지 그룹 | **Ctrl+Shift+F3** |
| 이동 / 다른 창에 정렬 | 드래그 / **Shift+드래그** |
| 여러 이미지 선택 / 전체 선택 | **Ctrl+클릭 / Ctrl+A** |
| 배율 변경 | 휠, **+ / -**, 테두리 드래그 |
| 불투명도 변경 | **Ctrl+휠**, **Ctrl + (+ / -)** |
| 배율·불투명도 100% | 가운데 클릭 |
| 시계 / 반시계 90° 회전 | **1 / 2** |
| 좌우 / 상하 반전 | **3 / 4** |
| 같은 이미지 위치에서 주석 편집 | **Space** |
| 접기 / 펼치기 | **Shift+더블클릭** |
| 활성 플로팅 창 닫기 | 설정한 닫기 단축키(기본 **Esc**), **Ctrl+W**, 더블클릭 |
| 이미지 완전히 제거 | **Shift+Esc** |
| 클립보드로 이미지 교체 | **Ctrl+V** |
| 원문 텍스트 복사 | **Ctrl+Shift+C** |
| 설정 열기 | **Ctrl+Shift+P** |

이미지·GIF·텍스트·기본 HTML/RTF 서식·HEX/RGB 색상·이미지 파일을 붙일 수 있습니다.
플로팅 단축키는 캡처 중이면 현재 선택 영역을, 편집창이 활성 상태이면 편집 중인 이미지를 우선 띄웁니다.
캡처 중 선택한 영역이 없으면 아무 이미지도 띄우지 않습니다. 캡처·편집 중이 아닐 때는 최근 닫은 이미지를 복구하거나 클립보드를 붙입니다.
플로팅 단축키를 해제하면 F3도 동작하지 않으며, 캡처·편집창의 기본 **Ctrl+T**는 계속 사용할 수 있습니다.
**활성 플로팅 창 닫기**는 해당 이미지가 활성 상태일 때만 동작하는 단축키입니다. 설정 → 단축키에서 변경하거나 해제할 수 있습니다.
이미지 또는 도구 모음을 클릭한 뒤 닫기 키를 누르세요. 닫기 키를 해제해도 **Ctrl+W**, 더블클릭과 닫기 버튼은 사용할 수 있습니다.
같은 이미지 파일을 다시 붙이면 파일 경로를 텍스트 이미지로 표시합니다(설정에서 끌 수 있음).
닫은 이미지 복구 개수 기본값은 1개이며, 전체 숨기기는 이 개수에 영향을 주지 않습니다.
클릭 통과를 끄려면 이미지가 없는 곳에서 해당 단축키를 누르거나 트레이의 표시·클릭 통과 해제를 사용하세요.

GIF는 우클릭 메뉴에서 재생·일시정지, 이전·다음 프레임, 속도를 조절합니다.
현재 프레임은 PNG로 저장하며, 원본 GIF와 변환·재생 상태는 다음 실행을 위해 별도로 보관합니다.
그룹 관리에서 생성·이름 변경·전환, 선택 이미지 이동, `.chacha` 그룹 파일 내보내기·가져오기를 지원합니다.
그룹 파일은 Chacha 형식이며 Snipaste 그룹 파일과 호환되지 않습니다.

## 설정과 보관

설정은 단축키 / 캡처 / 저장·기록 / 고정·시작 탭으로 구성됩니다.
단축키 입력란을 클릭하면 **키를 눌러 주세요…**가 나타납니다. 실제 키 조합을 누르면 즉시 표시되고 **설정 저장**으로 적용합니다.
입력란 주변의 둥근 테두리를 클릭해도 키 입력을 시작합니다. **해제**로 기능별 단축키를 지울 수 있고, 전부 비워 두어도 됩니다.
설정 중에는 앱 전역 단축키를 잠시 해제하므로 F1/F3도 캡처를 시작하지 않고 입력할 수 있습니다.
입력란에 포커스가 있는 동안에는 다른 앱이 등록한 키도 조합으로 읽습니다. 저장 전에 전역 등록 충돌을 확인하고 해당 항목을 안내합니다.
Snipaste 같은 다른 캡처 앱과 함께 실행할 때는 겹치는 단축키를 지우거나 다른 키로 지정하세요. 등록에 실패한 키는 대시보드에 표시합니다.
5개 전역 단축키와 활성 플로팅 창의 닫기 단축키, 커서 포함, 창·요소 자동 감지, 다른 창 활성화 시 캡처 취소, 자동 저장,
두 저장 폴더, 기록 개수(1–200), 닫은 이미지 보관(0–100), HTML과 경로 붙여넣기를 변경합니다.
Windows 로그인 시 실행도 설정에서 선택할 수 있습니다.

- 설정·기록·고정 이미지: `%LOCALAPPDATA%\ChachaCapture`
- 기본 저장·빠른 저장: Windows 사진 폴더의 `Chacha Capture`
- 오류 로그: `%LOCALAPPDATA%\ChachaCapture\error.log`
- 최근 캡처 진단: `%LOCALAPPDATA%\ChachaCapture\capture-diagnostics.txt`
- 업데이트 확인·다운로드에만 GitHub를 사용합니다. 캡처 이미지 업로드나 계정 로그인 기능은 없습니다.

## 명령행

실행 중인 앱에도 명령을 전달합니다. 경로와 공백이 있는 문자는 따옴표로 감싸세요.

```powershell
.\ChachaCapture.exe snip
.\ChachaCapture.exe snip --full -o clipboard
.\ChachaCapture.exe snip --area 100 100 640 480 --delay 1.5 -o pin
.\ChachaCapture.exe snip --last -o quick-save
.\ChachaCapture.exe snip --active-window -o "C:\Pictures\window.png"
.\ChachaCapture.exe paste --plain "메모 내용" --pos 100 100
.\ChachaCapture.exe paste --files "C:\Pictures\sample.gif"
.\ChachaCapture.exe whiteboard
.\ChachaCapture.exe switch-group "기본 그룹"
.\ChachaCapture.exe open-preferences
.\ChachaCapture.exe exit
```

추가 명령: `toggle-images`, `show-images`, `hide-images`, `create-group`, `switch-groups`,
`show-group-manager`, `empty-group`, `toggle-click-through`, `no-click-through`, `show-tray-menu`.
캡처 옵션: `--full`, `--last`, `--area`, `--size`, `--active-window`, `--active-screen`, `--custom`, `--delay`.
출력: `pin`, `clipboard`, `quick-save`, `file-dialog`, `printer`, `success`, 파일 경로.
실행 옵션: `--tray`, `--open FILE`, `--data-dir DIRECTORY`.

## 호환 범위

v1.4.1은 Ctrl+C 복사 후 캡처·편집·플로팅 창이 남는 문제를 수정한 버전입니다. **Snipaste 전체 제품과 완전한 1:1 일치를 보증하는 버전은 아닙니다.**
UI는 Chacha 한국어 테마이며, 세부 차이는 [기능 대조표](https://github.com/youkdonghun/chacha/blob/main/docs/PARITY.md)에 명시했습니다.
OCR·바코드·가상 데스크톱 연동 같은 PRO 기능, 모든 HTML/CSS의 동일 렌더링, Snipaste 설정·그룹 파일 호환은 포함하지 않습니다.
UI 요소 감지는 대상 앱의 접근성 정보 제공 여부에 따라 달라집니다.
회사 보안 프로그램과의 모든 조합을 검증한 것은 아닙니다. Windows가 접근을 거부하거나 보호된 콘텐츠로 가린 화면은 우회하지 않습니다.
HDR 색 관리와 실제 혼합 DPI 다중 모니터는 별도 검증이 필요합니다.

캡처 API의 동작과 제약은 Microsoft의 [BitBlt](https://learn.microsoft.com/en-us/windows/win32/api/wingdi/nf-wingdi-bitblt),
[Desktop Duplication](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_2/nf-dxgi1_2-idxgioutput1-duplicateoutput),
[프레임 정보](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_2/ns-dxgi1_2-dxgi_outdupl_frame_info) 문서를 참고하세요.
Desktop Duplication은 기본적으로 한 세션에서 동시에 사용하는 프로세스 수에 제한이 있습니다.

## 빌드

```powershell
.\build.ps1 -Test
```

Windows 내장 Framework64 C# 5 컴파일러를 사용합니다. .NET SDK와 NuGet 설치는 필요하지 않습니다.
결과: `dist\ChachaCapture.exe`, `dist\ChachaCapture-1.4.1-win-x64.zip`, SHA-256, 자체 검사 결과.
[검증 기록](docs/VALIDATION.md)과 [변경점](docs/RELEASE-v1.4.1.md)을 참고하세요.

실제 화면 전체 검사는 `tests/run-desktop-tests.ps1`로 실행합니다. 화면 접근이 제한된 세션에서는 `-WorkflowOnly`로 바탕화면 캡처를 제외한 컨트롤러 흐름만 검사할 수 있으며, 전체 화면 검사를 대체하지 않습니다.

MIT License. 작성자: youkdonghun.
