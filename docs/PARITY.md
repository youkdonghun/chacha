...
| 완료 | Ctrl+C·복사 버튼은 복사 후 닫기, Enter·더블클릭은 자동 플로팅 설정 적용, 지정 플로팅 키/Ctrl+T로 현재 이미지만 플로팅, 저장·빠른 저장·인쇄 | Ctrl+C는 활성 이미지에만 적용; 명시적 플로팅은 클립보드·빠른 저장·캡처 기록을 변경하지 않음; 재실행 복원 캐시는 유지 |
