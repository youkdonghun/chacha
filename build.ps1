...
& $taskCompiler /nologo /target:winexe /platform:x64 /optimize+ /langversion:5 /codepage:65001 /warn:4 "/out:$taskExe" "/win32manifest:$(Join-Path $taskRoot 'src\app.manifest')" "/win32icon:$taskIcon" /reference:System.dll /reference:System.Core.dll /reference:System.Drawing.dll /reference:System.Windows.Forms.dll /reference:System.Xml.dll /reference:System.IO.Compression.dll /reference:System.IO.Compression.FileSystem.dll "/reference:$taskFramework\WPF\UIAutomationClient.dll" "/reference:$taskFramework\WPF\UIAutomationTypes.dll" "/reference:$taskFramework\WPF\WindowsBase.dll" $taskSources
...
Copy-Item -LiteralPath @((Join-Path $taskRoot 'docs\PARITY.md'), (Join-Path $taskRoot 'docs\VALIDATION.md'), (Join-Path $taskRoot 'docs\RELEASE-v1.1.0.md')) -Destination $taskDocs -Force
...
$taskArchive = Join-Path $taskOutput 'ChachaCapture-1.1.0-win-x64.zip'
