param([switch]$SkipBuild, [switch]$WorkflowOnly)
...
$taskReport = Join-Path $taskRoot 'artifacts\desktop-integration-v1.4.1.txt'
if ($WorkflowOnly) { $taskReport = Join-Path $taskRoot 'artifacts\workflow-integration-v1.4.1.txt' }
...
$taskArguments = @('"' + $taskReport + '"')
if ($WorkflowOnly) { $taskArguments += '--workflow-only' }
